...
void	to_direct_here_doc(t_args arg, int ac, int *p)
{
	if (ac == 3)
	{
...
void	childs_here_doc(t_args arg, char **paths, int ac, int *p)
{
	char	**args;
	pid_t	cpid;
...
void	here_doc_init(t_args arg, int **pipefd, char **limiter)
{
	int	i;
...
void	here_doc(t_args arg, char **paths)
{
	...
	here_doc_init(arg, &pipefd, &limiter);
	line = "";
	while (1)
	{
		write(1, "heredoc> ", 9);
	...
	ac = 2;
	while (++ac < arg.argc - 1)
		childs_here_doc(arg, paths, ac, pipefd);
	to_close(pipefd, arg);
}
